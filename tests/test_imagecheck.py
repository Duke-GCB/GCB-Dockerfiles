...
class TestRunFunctions(unittest.TestCase):
    ...
    def test_build_and_push_docker_image(self, mock_run_bash_cmd):
    ...
    @patch('imagecheck.run_bash_cmd')
    def test_run_singularity_get_output(self, mock_run_bash_cmd):
        mock_run_bash_cmd.return_value = "/tmp/somedir"
        result = imagecheck.run_singularity_get_output("someimage", "pwd")
        self.assertEqual(result, "/tmp/somedir")
        mock_run_bash_cmd.assert_has_calls([
            call('SINGULARITY_NOHTTPS=yes singularity exec docker://someimage pwd 2>&1',
                 ignore_non_zero_exit_status=True)
        ])

    @patch('imagecheck.get_test_list')
    @patch('imagecheck.run_docker_get_output')
    @patch('imagecheck.run_singularity_get_output')
    @patch('imagecheck.print_test_error')
    def test_run_tests_no_error(self, mock_print_test_error, mock_run_singularity_get_output,
        ...
        mock_run_singularity_get_output.return_value = "Usage: bwa"
    ...
    @patch('imagecheck.run_singularity_get_output')
    @patch('imagecheck.print_test_error')
    def test_run_tests_docker_error(self, mock_print_test_error, mock_run_singularity_get_output,
                                    mock_run_docker_get_output, mock_get_test_list):
        mock_get_test_list.return_value = [
            ("bwa", ".*Usage: bwa.*")
        ]
    ...
    @patch('imagecheck.run_singularity_get_output')
    @patch('imagecheck.print_test_error')
    def test_run_tests_singularity_error(self, mock_print_test_error, mock_run_singularity_get_output,
                                         mock_run_docker_get_output, mock_get_test_list):
        mock_get_test_list.return_value = [
            ("bwa", ".*Usage: bwa.*")
        ]
        mock_run_docker_get_output.return_value = "Usage: bwa"
        mock_run_singularity_get_output.return_value = "Error: no such file"
        had_error = imagecheck.run_tests("someimage", "/tmp/fakefile.yml")
        mock_print_test_error.assert_has_calls([
            call('bwa', '.*Usage: bwa.*', 'Error: no such file')
        ])
